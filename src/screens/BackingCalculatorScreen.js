// BackingCalculatorScreen.js
// Calculates quilt backing yardage, accounting for seaming panels if needed.
...
export default function BackingCalculatorScreen() {
  ...
  function calculate() {
    ...
    const o = parseFloat(overlap) || 4;
    const fw = parseFloat(fabW) || 44;
    if (!w || !h) return;
    setResult(calcBacking({ quiltWidth: w, quiltHeight: h, overlap: o, fabricWidth: fw }));
  }
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={s.card}>
            ...
            <View style={s.row}>
              <View style={{ flex: 1 }}>
                <Field label="Overlap per side" value={overlap} onChangeText={v => { setOverlap(v); setResult(null); }} hint="4" />
              </View>
              <View style={s.rowGap} />
              <View style={{ flex: 1 }}>
          ...
          {result && (
            <View style={s.result}>
              <Text style={s.resultTitle}>Backing Plan</Text>
              <View style={s.resultRow}>
                <Text style={s.resultLabel}>Fabric panels</Text>
                <Text style={s.resultVal}>{result.panels}</Text>
              </View>
              {result.seams > 0 && (
                <View style={s.resultRow}>
                  <Text style={s.resultLabel}>Seams to join</Text>
                  <Text style={s.resultVal}>{result.seams}</Text>
                </View>
              )}
              ...
              {result.panels > 1 && (
                <Text style={s.note}>
                  Panels will be seamed side-by-side. Press seams open for a flatter finish.
                </Text>
              )}
            </View>
          )}
